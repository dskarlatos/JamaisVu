...
usage() {
    echo "usage: counter.sh [ -i | --max-insts INST ]
                [ -w | --warmup-insts INST ] [ --dry-run ]
                [ --no-clear ] [ -h | --help ] BENCHMARK SIMPT_ID"
    echo ""
...
PARSED_ARGUMENTS=$(getopt -a -n unsafe -o hi:w: --long \
help,max-insts:,warmup-insts:,dry-run,no-clear -- "$@")
...
OUTPUT_DIR=$GEM5_ROOT/output/Unsafe/$BENCHMARK/$SIMPT
